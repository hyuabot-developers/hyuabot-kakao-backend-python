...
def get_shuttle_departure(request):
    """셔틀 정류장을 사용자로부터 입력받아 행선지별 도착 정보를 최대 두개씩 반환합니다."""
    _, user_answer = get_user_data(json.loads(request.body))
...
def get_food_menu(request):
    ...
    user_id, answer = get_user_data(json.loads(request.body))
...
def get_reading_room_info(request):
...
def get_shuttle_stop_info(self):

...
def make_answer_bus_info(language):
    ...
    if language == 'Korean':
        ...
        string += '3102(Guesthouse→Gangnam)\n'
        if realtime_info['3102']:
            arrival_info = realtime_info['3102'][0]
            string += f"{arrival_info['location']} stop left/arriving after {arrival_info['time']} mins({arrival_info['seat']} seats)\n"
        elif timetable['3102']:
            arrival_info = timetable['3102'][0]
            string += f"Departing at {arrival_info['time'].strftime('%H:%M')} from terminal stop\n"
        else:
            string += 'There is no bus to come\n'

        string += '\n10-1(Guesthouse→Sangnoksu Stn)\n'
        if realtime_info['10-1']:
            arrival_info = realtime_info['10-1'][0]
            string += f"{arrival_info['location']} stop left/arriving after {arrival_info['time']} mins\n"        
        elif timetable['10-1']:
            arrival_info = timetable['10-1'][0]
            string += f"Departing at {arrival_info['time'].strftime('%H:%M')} from terminal stop\n"
        else:
